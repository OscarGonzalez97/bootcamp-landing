    ...
    <div className='w-full fixed top-0 left-0'>
      <div className='md:flex h-32 justify-between bg-secondary-foreground py-7 md:px-10 px-7'>
        <div className='text-2xl cursor-pointer lg:flex items-center text-white font-[Arial]'>
          <img src='https://www.roshka.com/static/img/all-white-logo.svg' className='w-72'/>
        </div>
        ...
        <div className={`md:hidden ${showMobileOptions ? '' : ''}`}>
          <button className='text-2xl hover:text-slate-400 cursor-pointer flex items-center text-white font-[Arial] mt-14' onClick={handleToggleMobileOptions}>
            {showMobileOptions ? "☰" : "☰"}
          </button>
        </div>
        {showMobileOptions && (
